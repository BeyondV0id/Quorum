...
import { chambers, chamberMembers } from "../db/schema.js";
...
router.get("/", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  const rows = await db.query.chambers.findMany({
    extras: spaceExtras(username!),
    where: q ? ilike(chambers.name, `%${q}%`) : undefined,
    orderBy: [chambers.name],
  });
...
router.post("/", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const [space] = await db.insert(chambers).values({ name, description, creatorUsername: username, colorIndex: colorIndex ?? 0 }).returning();
    await db.insert(chamberMembers).values({ chamberUid: space.uid, username: username! });
...
router.delete("/", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  await db.delete(chambers).where(and(eq(chambers.name, name), eq(chambers.creatorUsername!, username!)));
...
router.patch("/:uid", requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const c = await db.query.chambers.findFirst({ columns: { creatorUsername: true }, where: eq(chambers.uid, req.params.uid) });
    ...
    await db.update(chambers).set({ name, description, colorIndex: colorIndex ?? 0 }).where(eq(chambers.uid, req.params.uid));
...
router.post("/:uid/join", requireAuth, async (req: Request, res: Response): Promise<void> => {
  const { username } = (req as AuthRequest).user;
  try {
    await db.insert(chamberMembers).values({ chamberUid: req.params.uid, username: username! }).onConflictDoNothing();
...
router.post("/:uid/leave", requireAuth, async (req: Request, res: Response): Promise<void> => {
  const { username } = (req as AuthRequest).user;
  await db.delete(chamberMembers).where(and(eq(chamberMembers.chamberUid, req.params.uid), eq(chamberMembers.username, username!)));
